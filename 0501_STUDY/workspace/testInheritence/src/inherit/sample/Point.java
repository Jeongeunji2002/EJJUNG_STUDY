...
public class Point {
	...
	// 클래스에 생성자가 하나도 없으면 JVM이 기본 생성자 자동 추가하면서 컴파일함 
	// 상속에 사용될 부모클래스일 때는 기본생성사 반드시 있어야 함
	public Point() {
		System.out.println("Point 기본 생성자 this : "+this.hashCode());
	} 	// JVM이 준비해놓은 기본값으로 초기화됨
	...
	public Point(double x, double y) {
		...
		// new 할 때 자유메모리 heap 에 할당되는 객체의 주소를 자동 전달받음
	...
	// 오버라이딩 규칙1 : 부모 메소드의 시그니쳐(Signiture) 그대로 사용해야 함, 메소드 안에 실행내용만 변경함
	...
	public boolean equals(Object obj) {
		...
		// 두 레퍼럼스가 참조하는 객체 안의 필드 값이 같은지로 변경함
		
		// 메소드 사용 시 Point 객체의 주소를 전달하면, Object 타입의 매개변수가 받게 됨 : 다형성 적용됨
		// 그런데 Object obj 는 Point 가 가진 x 에 접근할 수 없다 : obj 에는 x가 없음
