...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%-- jstl의 core 태그라이브러리 사용하려면, 아래의 구문을 사용해야 함--%>
...
<body>
	<h1>JSTL : JSP Standard Tag Library 테스트 파일</h1>
	...
	<br> 전송온 값 출력 :
	<c:out value="${param.name}" default="손님" />
	<br>
	...
	<h4>c:choose 태그 : swich 문과 같음</h4>
	<c:set var="no" value="3" />
	<c:choose>
		<c:when test="${ no eq 1 }">반갑습니다.</c:when>
		<c:when test="${ no eq 2 }">안녕하세요.</c:when>
...
${status.count } : ${book} <br>
	...
	java.lang.String 의 split() 메소드와도 같은 기능임
	<br>
	<ul>
		<c:forTokens items="yello,pink,blue,green,red" delims="," var="clr">
			<li>${clr }</li>
		</c:forTokens>
	</ul>
	<hr>
...
${lib } <br>
	...
	<%-- spring 웹에플리케이션에서는 직접 jsp에서 jsp로 패이지 이동 못 함
	 반드시 서버 컨트롤러를 거쳐서 뷰리졸버로 리턴된 뷰파일명으로 페이지가 출력되어야 함
--%>
	<!-- <a href="/testjstl/WEB-INF/views/test/testFmt.jsp?no=123&name=홍길동&pahe=1">testFmt 로 이동</a><br> -->
	<a href="moveFmt.do?no=${num1 }&name=홍길동&page=${num2 }">testFmt 로
		이동</a>
	<br>
	<%--위에 url 요청과 전송 파라메타를 별도로 다루기 위해 c:url 태그 사용할 수 있음 --%>
	<c:url var="movef" value="moveFmt.do">
		<c:param name="no" value="${num1 }" />
		<c:param name="name" value="홍길동" />
		<c:param name="page" value="${num2 }" />
	</c:url>
	<a href="${movef }">testFmt로 이동 : c:url 사용</a>
	<hr>

	<h4>c:import 태그 : 별도로 작성된 jsp 파일을 포함시킬 때 사용</h4>
	jsp 지시자 태그의 include 와 같음
	<br>
	...
	<c:if test="${not empty e }">
		<%-- e !=null 과 같음, 예외가 발생했다면--%>
에러 메세지 : ${e.message } <br>
	</c:if>
	<hr>

	<h4>c:redirect 태그 : response.sendRedirect("내보낼뷰 또는 연결할 요청 ulr")</h4>
