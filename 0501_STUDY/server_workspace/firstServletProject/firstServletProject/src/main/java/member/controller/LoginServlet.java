 ...
 */
@WebServlet("/login")  // WEB-INF/web.xml 에 서블릿 자동 등록 설정하는 어노테이션임
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request 매개변수 : 
		//사용자 브라우저 뷰페이지 input 으로 입력된 값 또는  쿼리스트링 (?이름=전송값&이름=전송값&....)으로 전달된 값을 
		...
		//로그인 처리용 컨트롤러 ----------------------------------------------------
		//컨트롤러의 코드 작성 순서와 내용은 비숫함 => 웹의 흐름을 빨리 파악해야 함
		
		//1. 전송온 값에 한글이 있다면 인코딩 처리함 (전송온 값에 한글이 없다면 생략해도 됨)
		request.setCharacterEncoding("utf-8");  //view 페이지의 문자셋과 맞춰줌
		
		//2. 전송온 값 꺼내서, 변수 또는 dto 객체에 저장 처리함 : request.getParameter("전송보낸이름") : String 반환됨
		...
		try {
			...
			if(loginUser != null) {  //성공시
				...
				response.sendRedirect("index.jsp");  //"/first/index.jsp" 가능함
				...
				request.setAttribute("message", "로그인 실패! 아아디와 암호를 다시 확인하세요.");
			...
			e.printStackTrace();
			/*
			 * //내보낼 뷰 지정함 : 상대경로만 사용 가능함 RequestDispatcher view =
			 * request.getRequestDispatcher("views/common/error.jsp"); //뷰와 함께 내보낼 데이터 처리 :
			 * request 객체에 저장함 => Map 구조임 (key, value) //setAttribute("key", value object);
			 * 사용함 request.setAttribute("message", "로그인 실패! " + e.getMessage());
			 * 
			 * //로그인을 요청한 클라이언트로 내보냄 view.forward(request, response);
			 */
			//로그 분석을 위해서 로그 파일을 만들고, 로그 파일에 로그로 에러정보를 기록 처리하는 것을 권장함
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		doGet(request, response);  //doGet() 메소드 쪽으로 전송온 정보를 념김
	}
